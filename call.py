...
def meter():
        ...
        powerloss=str(truncate(abs(integrate(ideal,5)-integrate(row,5))*0.017/1000,2))
        ...
        if ping('192.168.0.86')==False:
                templateData = {'volt' : '-','amp' : '-','watt' : '-','state' : 'Offline - GridDown / Unreachable','total' : str(d[day])+'  kWh','today': str(truncate(d[day]-d[day-1],2))+'  kWh'}
                legend='Daily Units'
