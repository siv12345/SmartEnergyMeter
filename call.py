...
def meter():
	...
	powerloss=str(truncate(abs(integrate(ideal,10)-integrate(row,10))*0.017/1000,2))
	...
	if ping('192.168.0.86')==False:
		templateData = {'volt' : '-','amp' : '-','watt' : '-','state' : 'Offline - GridDown','total' : str(d[day])+'  kWh','today': str(truncate(d[day]-d[day-1],2))+'  kWh'}
		legend='Daily Units'
		return render_template('index.html', loss=powerloss+" kWh", ideal=ideal, values=values, labels=labels, legend=legend, x=col, y=row, **templateData)
	...
	switch='Online' if data['18']!=0 else 'Offline'
